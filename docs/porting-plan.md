...
| 3b | Structure & xRFC codecs (classic-structure, classic-xrfc, recursive xRFC) | `src/values/{classic-structure,classic-xrfc,recursive-*}.ts` | flat structures + tables **done & verified live**; `classic-xrfc` (flat) and `recursive-xrfc` (nested structures/tables over the metadata graph) **done** (`internal/xrfc`, oracle+fuzz); `recursive-classic-xrfc` remains |
