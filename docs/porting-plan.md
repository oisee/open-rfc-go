...
| 3b | Structure & xRFC codecs (classic-structure, classic-xrfc, recursive xRFC) | `src/values/{classic-structure,classic-xrfc,recursive-*}.ts` | flat structures + tables **done & verified live**; `classic-xrfc` (flat STRING/XSTRING xRFC XML) **done** (`internal/xrfc`, oracle+fuzz); recursive/nested xRFC (`recursive-xrfc`, `recursive-classic-xrfc`) remains |
