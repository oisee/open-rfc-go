...
| Constant | Meaning |
|---|---|
| node suffix `e1 00 00 00 ac 11 00 03` | last 8 bytes = RFC node id + host IP (192.0.2.30); locates a GUID in a frame |
...
| `internal/rfcserver/replay.go` | `rfcGUIDNodeSuffix` `e1000000c000021e`, `swapRFCGUID` (structured swap), `convIDOf`/`withConvID` (conv id at offset 40) |
